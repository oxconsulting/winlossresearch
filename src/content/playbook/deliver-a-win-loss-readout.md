---
title: "How to Deliver a Win/Loss Readout"
description: "Run the readout as a live, cross-functional meeting built around synthesized findings, ending in a specific action per function."
phase: "report-distribute"
phaseOrder: 1
relatedPillars: ["how-to-build-win-loss-program", "win-loss-gtm-strategy"]
draft: false
---
...
1. [Structure the deck around synthesized findings, not individual deals](#step-1-structure-the-deck-around-synthesized-findings-not-individual-deals)
2. [Decide who needs to be in the room](#step-2-decide-who-needs-to-be-in-the-room)
3. [Run the meeting to end in action items, not just a presentation](#step-3-run-the-meeting-to-end-in-action-items-not-just-a-presentation)
4. [Close each finding with a specific ask per function](#step-4-close-each-finding-with-a-specific-ask-per-function)
5. [Follow up within two to three weeks](#step-5-follow-up-within-two-to-three-weeks)
...
Building this per-function translation for every finding is its own task with its own template; the full method for mapping a finding type to the right action per function is covered in [how to translate win/loss findings into function-specific actions](/playbook/translate-win-loss-findings-into-function-specific-actions/).
...
An action item still unstarted at the two-to-three-week mark is worth a direct conversation with its owner rather than a quiet extension. A commitment made out loud in a room with an executive sponsor present and then dropped without explanation is a signal about how seriously the program is being treated organizationally, not just a scheduling slip, and it's worth surfacing before the pattern repeats across the next cycle's readout.

## Related

### Other Plays in This Phase
- [How to Translate Win/Loss Findings into Function-Specific Actions](/playbook/translate-win-loss-findings-into-function-specific-actions/)
